...
require_once $_SERVER['DOCUMENT_ROOT'].'theme-app/botapi/YaDisk/tmebot/Configuration/TelegramBotHandMadeConfiguration.php';
require_once $_SERVER['DOCUMENT_ROOT'].'theme-app/botapi/YaDisk/tmebot/Controller/CallBackController.php';
require_once $_SERVER['DOCUMENT_ROOT'].'theme-app/botapi/YaDisk/tmebot/Controller/TextControler.php';
require_once $_SERVER['DOCUMENT_ROOT'].'theme-app/botapi/Configuration/DateBase.php';

include($_SERVER['DOCUMENT_ROOT'].'theme-app/botapi/YaDisk/tmebot/vendor/autoload.php'); //Подключаем библиотеку
...
$path = $_SERVER['DOCUMENT_ROOT'].'/theme-app/botapi/YaDisk/tmebot/';
...
if($chat_id == $botApiConfiguration->getManagerId() || $botApiConfiguration->getManagerIdSecond())
{
    ...
    if (!is_null($photo))
    {
        ...
        if (!empty($result['message']['photo'])) {
            ...
            if ($res['ok']) {
                ...
                $dest = $_SERVER['DOCUMENT_ROOT'].'/theme-app/botapi/YaDisk/photoUsers/'. basename($src);
    ...
    if (!is_null($file))
    {
        ...
        if (!empty($result['message']['document'])) {
            ...
            if ($res['ok']) {
                $src  = 'https://api.telegram.org/file/bot' . $token . '/' . $res['result']['file_path'];
                $dest = $_SERVER['DOCUMENT_ROOT'].'/theme-app/botapi/YaDisk/fileUsers/'. basename($src);
